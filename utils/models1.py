    ...
    def train(self, batch_size, num_epochs=25, model_save_path='best_train_model.pth'):
        ...
        for epoch in range(self.num_epochs):
            ...
            if round(self.val_loss_values[-1], 4) < round(self.best_loss, 4):
                ...
                print(f'Validation loss has not improved for {round(0.05*self.num_epochs,0)} epochs, stopping training.\n')
                break
...
if __name__ == '__main__':
    if os.path.exists('DAPI_train.csv') and os.path.exists('DAPI_test.csv'):
        train_val_df = pd.read_csv('DAPI_train.csv')
        test_df = pd.read_csv('DAPI_test.csv')
    ...
    model.train(batch_size=4, num_epochs = 200, model_save_path='best_tuned_model.pth')
