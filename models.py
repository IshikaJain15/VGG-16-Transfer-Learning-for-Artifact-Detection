...
class ImageClassifier(object):
    ...
    def __init__(self, train_df, val_df, test_df, params, train_strat='feature_extraction'):
        ...
        self.model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=2)

# Example: Optionally add a softmax layer if CrossEntropyLoss is used as the loss function
        self.model.classifier.add_module('7', torch.nn.LogSoftmax(dim=1))
    ...
    def get_activation_map(self, inputs):
    ...
    # Get the activation map
        ...
        # Forward pass to get the model outputs
            outputs = self.model(inputs)
        # Extract the feature map from a specific layer (e.g., the last convolutional layer)
            activation = self.model.get_activation(outputs)
        return activation
    ...
    def test(self, load_from_path=False, model_path=None):
        ...
        with torch.no_grad():
            ...
            for inputs, labels in self.test_loader:
                ...
                activation_map = self.get_activation_map(inputs)

        # Display activation map
                plt.imshow(activation_map[0], cmap='hot', interpolation='nearest')
...
if __name__ == '__main__':
    ...
    '''model.train(batch_size=32, num_epochs = 200, model_save_path='best_tuned_model.pth')'''
    if os.path.exists('last-trained.pth'):
        model.test(load_from_path=True, model_path='last-trained.pth')
